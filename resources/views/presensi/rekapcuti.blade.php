@extends('layouts.presensirekapcuti')
...
@section('header')
    ...
    <div class="appHeader bg-primary text-light">
        ...
        <div class="pageTitle">Data Cuti Karyawan</div>
        <div class="right"></div>
    </div>
@endsection
    ...
    <div class="row">
        <div class="col">
            @forelse ($datarekap as $d)
                @if ($d->status === 'Cuti')
                    <ul class="listview image-listview" style="margin-top: 3px">
                        <li class="show-detail" data-id_formulir="{{ $d->id_formulir }}"
                            ...
                            data-tgl_mulai="{{ $d->tgl_mulai }}" data-tgl_selesai="{{ $d->tgl_selesai }}"
                            data-jenis_cuti="{{ $d->jenis_cuti }}" data-lama_cuti="{{ $d->lama_cuti }}"
                            data-keterangan="{{ $d->keterangan }}" data-bukti="{{ $d->bukti }}"
                            data-bukti-url="{{ $d->bukti ? asset('storage/upload/bukti/' . $d->bukti) : '' }}">
                            <div class="item">
                                <div class="in">
                                    <div>
                                        <b>{{ date('d-m-Y', strtotime($d->tgl_mulai)) }}</b><br>
                                        <small class="text-muted">{{ $d->jenis_cuti }}</small>
                                    </div>
                ...
                @endif
            @empty
                <center><span style="font-weight:700"><br><br><br><br><br><br><br><br><br><br><br>Belum ada data cuti</span>
                </center>
            @endforelse
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title w-100 text-center fw-bold" id="detailModalLabel">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.show-detail').forEach(el => {
                el.addEventListener('click', () => {
                    ...
                    const id_formulir = d.id_formulir;
                    ...
                    if (d.status === 'Cuti') {
                        ...
                        html += `<p><strong>Lama Cuti:</strong> ${d.lama_cuti} Hari</p>`;
                    ...
                    const editUrl = `/formulir/editcuti/${d.id_formulir}`;
                    editButton.href = editUrl;

                    // Set URL untuk button Delete
                    deleteForm.action = `/formulir/deletecuti/${d.id_formulir}`;
